...
export default function FabricContext() {
    const { id } = useParams();
    const product = fabrics.find((item) => item.id === id);
    ...
    const related = fabrics.filter((item) => item.category === product.category);
            ...
            <div className="px-4 w-full pt-24">
                <div className="flex items-center justify-center">
                    <div className="flex flex-col md:flex-row">
                        ...
                        />
                        <div className="px-3 md:ml-8 md:w-3/4 mt-4 md:mt-0 inter">
                            ...
                            <p className="text-gray-700 text-md font-normal mt-3">Price (yards): &#8358;{product.price}</p>
                            <div className="mt-3">
                                <form action="">
                                    <div>
                                        <input
            ...
            <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-6 py-2 px-3 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                {related.length > 0 ? (
                    related.map((relatedProduct) => (
                        ...
                        >
                            <Link
                                to={`/fabric/${relatedProduct.id}/${relatedProduct.name}`}
                            >
